...
N_FIXED = 0
N_OPT = 418
lambda_scalar = 2*N_OPT + N_FIXED # a car be considered at most two times (the maximum length of a path is 2)

print(f"The problem has {N_OPT*3} variables")
print(f"The number of feasible solution is {2**(N_OPT*3)}")
dtab = pd.read_excel('second_small_graph_structure.xlsx')
# print(dtab.head())
...
paths_3 = [['origin','middle', 'destination'], ['origin', 'a', 'destination'], ['origin', 'b','destination']]
paths = []
